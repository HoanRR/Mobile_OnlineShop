...
public class UserService {
    ...
    public UserDetailResponse getUser(Long id) {
        User user = userRepository.findByUserId(id)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND, "Không tìm thấy user có ID: " + id));
    ...
    public MessageResponse createAccount(CreateAccountRequest request) {
        ...
        if (isAutoGenerated) {
            return MessageResponse.builder().message("Tạo tài khoản thành công! Mật khẩu tự động là: " + rawPassword)
                    .build();
        }
